...
final case class PackagedProduct(
  remainingShelfLife: Float,
  value: Float,
  ...
  override def toString() =
    s"Product (name:${order.name}, temp:${order.temp}, remainingShelfLife:$remainingShelfLife, value:$value, " +
      s"createdOn:${createdOn.format(dateFormatter)}, lastUpdatedOn:${updatedOn
        .format(dateFormatter)}, orderId:${order.id})"

  def prettyString =
    s"Product (${order.name} temp:${order.temp} remainingShelfLife:$remainingShelfLife, value:$value)"
  ...
  /** Creates a copy of the packaged product representing the updated state in time assuming it stayed in
    ...
    */
  def phantomCopy(
    shelfDecayModifier: Float,
    current: LocalDateTime = LocalDateTime.now()
  ): PackagedProduct = {
    val duration = ChronoUnit.MILLIS
      .between(updatedOn, current)
      .toFloat / MillliSecondsInOneSecond
    if (duration > 0) {
      val remainingLife = math.max(
        0,
        if (remainingShelfLife <= 0) 0
        else
          (remainingShelfLife - duration - order.decayRate * duration * shelfDecayModifier)
      )
      val newValue = math.max(
        0,
        if (order.shelfLife <= 0) 0f
        else remainingLife.toFloat / order.shelfLife
      )
  ...
  /** How many more millis needs to pass before expected before pickup: (lowerBound,upperBound)
    */
  def pickupWindowInMillis(
    time: LocalDateTime = LocalDateTime.now()
  ): (Long, Long) = {
    val soonestExpectedArrival = createdOn.plus(
      Courier.EarliestDeliveryAfterOrderReceivedInSeconds,
      ChronoUnit.SECONDS
    )
    val pickupTimeDiff = Duration.between(time, soonestExpectedArrival).toMillis
    (
      pickupTimeDiff,
      pickupTimeDiff + Courier.DeliveryTimeWindowSizeInSeconds * 1000
    )
  }
}
...
case object PackagedProduct {
  ...
  def apply(
    ...
    new PackagedProduct(order.shelfLife.toFloat, 1, time, time, order)
  }

  object IncreasingValue extends Ordering[PackagedProduct] {
    def compare(a: PackagedProduct, b: PackagedProduct): Int =
      Ordering
        .Tuple2(Ordering.Float.IeeeOrdering, Ordering.String)
        .compare((a.value, a.order.id), (b.value, b.order.id))
  }
}

private[storage] case class ExpirationInfo(
  product: PackagedProduct,
  ifInCurrentShelf: Long,
  ifMovedToTarget: Long,
  timeInMillisTillPickup: (Long, Long),
  shelf: Shelf
) {
  def wantsToMove(millisIntoFuture: Long) =
    !willExpireForSure(millisIntoFuture) && ifInCurrentShelf < ifMovedToTarget
  ...
  def okToMove(millisIntoFuture: Long) =
    !willExpireForSure(millisIntoFuture) && safeToMove(millisIntoFuture)

  private def safeToMove(millisIntoFuture: Long) =
    (ifMovedToTarget - millisIntoFuture) > timeInMillisTillPickup._1

}

private[storage] case class ProductPair(
  inOverflow: ExpirationInfo,
  inTarget: ExpirationInfo
) {

  /** Returns either one of the products that will expire for sure in given time, or None
    */
  def willExpireForSure(
    millisIntoFuture: Long
  ): Option[(Shelf, PackagedProduct)] = {
    if (inOverflow.willExpireForSure(millisIntoFuture))
      Some((overflow, inOverflow.product))
    else if (inTarget.willExpireForSure(millisIntoFuture))
      Some(target, inTarget.product)
    else None
  }
  ...
  /** Returns whether it is recommended to swap the products to ensure either one of the products will improve
    ...
    */
  def swapRecommended(threshold: Int): Boolean =
    inOverflow.wantsToMove(threshold) && inTarget.okToMove(threshold) ||
  ...
  /** Returns true if either of the products will expire for sure if not moved within a certain time frame 'threshold'
    */
  def inCriticalZone(threshold: Int): Boolean = inOverflow.willExpireForSure(
    threshold
  ) || inTarget.willExpireForSure(threshold)
}
...
private[storage] case object ProductPair {
  ...
  def apply(
    ...
    new ProductPair(
      ExpirationInfo(
        p1,
        p1.expirationInMillis(s1.decayModifier.toFloat),
        p1.expirationInMillis(s2.decayModifier.toFloat),
        p1.pickupWindowInMillis(),
        s1
      ),
      ExpirationInfo(
        p2,
        p2.expirationInMillis(s2.decayModifier.toFloat),
        p2.expirationInMillis(s1.decayModifier.toFloat),
        p2.pickupWindowInMillis(),
        s2
      )
    )
  }
