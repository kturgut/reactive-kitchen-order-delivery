...
object ShelfManager {
  ...
  def props(orderProcessorOption: Option[ActorRef]) = Props(
    new ShelfManager(orderProcessorOption)
  )

  // TODO Turn discard reason to trait
  case class DiscardOrder(
    order: Order,
    reason: String,
...
class ShelfManager(orderProcessorOption: Option[ActorRef] = None)
  ...
  timers.startSingleTimer(
    TimerKey,
    StartAutomaticShelfLifeOptimization,
    100.millis
  )
  ...
  def readyForService(
    courierAssignments: ListMap[Order, CourierAssignment],
    storage: Storage
  ): Receive = {
    ...
    case ManageProductsOnShelves =>
      val updatedAssignments = publishDiscardedOrders(
        storage.optimizeShelfPlacement(),
        courierAssignments
      )
...
//        storage.reportStatus(true)
    ...
    case product: PackagedProduct =>
      ...
      val updatedAssignments = publishDiscardedOrders(
        storage.putPackageOnShelf(product),
        courierAssignments
      )
      context.become(readyForService(updatedAssignments, storage.snapshot()))
    ...
    case pickupRequest: PickupRequest =>
      log.debug(
        s"Shelf manager received pickup request ${pickupRequest.assignment.prettyString}"
      )
    ...
    case assignment: CourierAssignment =>
      log.debug(
        s"Shelf manager received assignment: ${assignment.prettyString}"
      )
      context.become(
        readyForService(
          (courierAssignments + (assignment.order -> assignment))
            .take(MaximumCourierAssignmentCacheSize),
          storage
        )
      )
  ...
  private def publishDiscardedOrders(
    discardedOrders: Iterable[DiscardOrder],
    courierAssignments: ListMap[Order, CourierAssignment]
  ): ListMap[Order, CourierAssignment] = {
    var assignments = courierAssignments
    discardedOrders.foreach(discardedOrder =>
      courierAssignments.get(discardedOrder.order) match {
        case Some(assignment: CourierAssignment) =>
          log.debug(
            s"Sending discarded order notice $discardedOrder to courier ${assignment.courierRef}"
          )
          assignment.courierRef ! discardedOrder
          log.debug(
            s"Sending discarded order notice $discardedOrder to orderProcessor ${orderProcessorOption.get}"
          )
          orderProcessorOption.foreach(_ ! discardedOrder)

          assignments -= assignment.order
        case _ =>
          orderProcessorOption.foreach(
            _ ! discardedOrder
          ) // notify order processor to update order lifecycle
      }
    )
    assignments
  }
