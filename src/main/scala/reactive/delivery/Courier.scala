...
object Courier {
  ...
  def props(name: String, orderProcessor: ActorRef, shelfManager: ActorRef) =
    Props(new Courier(name, orderProcessor, shelfManager))

  case class CourierAssignment(
    order: Order,
    courierName: String,
    courierRef: ActorRef,
    ...
    def prettyString =
      s"Courier $courierName is assigned to deliver order ${order.name} with id ${order.id}."
  }
  ...
  case class DeclineCourierAssignment(
    courierRef: ActorRef,
    product: PackagedProduct,
    originalSender: ActorRef
  ) extends JacksonSerializable

  case class DeliveryComplete(
    assignment: CourierAssignment,
    product: PackagedProduct,
    ...
    def prettyString(): String =
      s"DeliveryComplete of product '${assignment.order.name} in " + f"$durationOrderToDeliveryInSeconds%1.2f" +
        s" seconds. Value delivered:${product.value}. Tip received:${acceptance.tips}. Order id: ${assignment.order.id}"

    def durationOrderToDeliveryInSeconds: Float = Duration
      .between(product.createdOn, acceptance.time)
      .toMillis
      .toFloat / 1000
  }
...
class Courier(name: String, orderProcessor: ActorRef, shelfManager: ActorRef)
    extends Actor
    with ActorLogging {
  ...
  def onDelivery(
    ...
    case discard: DiscardOrder =>
      log.info(
        s"Cancelling trip for delivery for ${assignment.order.name} as product was discarded due to ${discard.reason}"
      )
    ...
    case DeliverNow =>
      ...
      val action =
        scheduledAction // !!? Do not use scheduleAction directly as future may be executed on different thread
      future.onComplete {
        case Success(None) =>
          log.info(
            s"Cancelling trip for delivery for ${assignment.order.name} as product was discarded. Reason unknown."
          )
          becomeAvailable(action)
        case Success(discard: DiscardOrder) => self ! discard
        case Success(pickup: Pickup) =>
          (assignment.order.customer ? DeliveryAcceptanceRequest(
            pickup.product.order
          )).onComplete {
            case Success(acceptance: DeliveryAcceptance) =>
              val delivery =
                DeliveryComplete(assignment, pickup.product, acceptance)
              log.info(delivery.prettyString())
              orderProcessor ! delivery
              becomeAvailable(action)
            case Success(message) =>
              log.error(
                s"Customer did not want to provide signature but sent this response: $message. THIS SHOULD NOT HAPPEN"
              )
              becomeAvailable(action)
            case Failure(exception) =>
              log.error(
                s"Exception received while waiting for customer signature. ${exception.getMessage}"
              )
          }
        case Failure(exception) =>
          log.error(
            s"Exception received while picking up package. Canceling delivery! Exception detail: ${exception.getMessage}"
          )
          becomeAvailable(action)
    ...
    case product: PackagedProduct =>
      log.warning(
        s"Courier $name received pickup order while already on delivery. Declining!"
      )
  ...
  def reminderToDeliver(courierActorRefToRemind: ActorRef): Cancellable = {
    val delay = randomizer
      .nextFloat() * DeliveryTimeWindowSizeInSeconds + EarliestDeliveryAfterOrderReceivedInSeconds
