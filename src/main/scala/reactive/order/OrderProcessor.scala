...
case object OrderProcessor {
  ...
  // EVENTS
  case class OrderRecord(time: LocalDateTime, order: Order)
      extends JacksonSerializable

  case class ProductRecord(time: LocalDateTime, product: PackagedProduct)
      extends JacksonSerializable

  case class DeliveryCompleteRecord(
    time: LocalDateTime,
    delivery: DeliveryComplete
  ) extends JacksonSerializable

  case class DiscardOrderRecord(time: LocalDateTime, discard: DiscardOrder)
      extends JacksonSerializable

  case class KitchenRelationshipRecord(name: String, actorPath: String)
      extends JacksonSerializable
...
class OrderProcessor extends PersistentActor with ActorLogging {
  ...
  override def receiveCommand: Receive = {
    ...
    case KitchenReadyForService(name, _, kitchenRef, _, _) =>
      ...
      persist(event) { eventRecorded =>
        log.info(
          s"A new kitchen named:${eventRecorded.name} is registered to receive orders at:${kitchenRef.path}."
        )
    ...
    case order: Order =>
      ...
      if (kitchens.isEmpty)
        ...
        persist(event) { eventRecorded =>
          ...
          log.info(
            s"Received ${orderCounter}th order on ${event.time}. Sending it kitchen:$order"
          )
    ...
    case product: PackagedProduct =>
      ...
      persist(event) { event =>
        ...
        updateState(
          event.product.order,
          (lifeCycle: OrderLifeCycle) => lifeCycle.update(event.product, log),
    ...
    case discard: DiscardOrder =>
      ...
      persist(event) { event =>
        ...
        log.debug(
          s"Order update: discarded: ${event.discard.order.name} with id ${event.discard.order.id}. Total discarded:$discardedOrderCounter"
        )
        updateState(
          event.discard.order,
          (lifeCycle: OrderLifeCycle) => lifeCycle.update(event.discard, log),
    ...
    case delivery: DeliveryComplete =>
      ...
      persist(event) { event =>
        ...
        updateState(
          event.delivery.assignment.order,
          (lifeCycle: OrderLifeCycle) => lifeCycle.update(event.delivery, log),
          () =>
            OrderLifeCycle(
              event.delivery.assignment.order,
              Some(event.delivery.assignment)
            )
        )
      }
    case ActorIdentity(name, Some(actorRef)) =>
      log.error(
        s"OrderProcessor re-establish connection with kitchen named $name"
      )
    ...
    case other =>
      log.error(
        s"Received unrecognized message $other from sender: ${sender()}"
      )
  }
  ...
  override def receiveRecover: Receive = {
    ...
    case RecoveryCompleted =>
      log.info(
        s"Recovering relationship with kitchen servicing at:${kitchenActorPath}."
      )
      context.actorSelection(kitchenActorPath) ! Identify(
        OrderProcessorActorName
      )
      if (orderCounter > 0) {
    ...
    case ProductRecord(date, product) =>
      ...
      updateState(
        product.order,
        (lifeCycle: OrderLifeCycle) => lifeCycle.update(product, log),
    ...
    case DiscardOrderRecord(date, discard) =>
      ...
      updateState(
        discard.order,
        (lifeCycle: OrderLifeCycle) => lifeCycle.update(discard, log),
    ...
    case DeliveryCompleteRecord(time, deliveryComplete) =>
      ...
      updateState(
        deliveryComplete.assignment.order,
        (lifeCycle: OrderLifeCycle) => lifeCycle.update(deliveryComplete, log),
        () =>
          OrderLifeCycle(
            deliveryComplete.assignment.order,
            Some(deliveryComplete.assignment)
          )
      )
  }
  ...
  def updateState(
    ...
    activeOrders = (activeOrders.get(order.id) match {
      case Some(lifeCycle) =>
        ...
        if (updatedEntry.isComplete) activeOrders
        else activeOrders + (order.id -> updatedEntry)
  ...
  def reportState(state: String) = {
    ...
    log.info(
      s"  Active orders pending production: ${activeOrders.values.count(!_.produced)}."
    )
    log.info(
      s"  Active Orders pending delivery: ${activeOrders.values.count(_.isComplete)}."
    )
  ...
  def reissueOrdersNotProcessed() = {
    activeOrders.values
      .filter(!_.produced)
      .map(_.order)
      .foreach(order => selectKitchenForOrder(kitchens, order) ! order)
  ...
  def selectKitchenForOrder(
    kitchens: Map[String, ActorRef],
    order: Order
  ): ActorRef = {
...
case object OrderHandlerManualTest extends App {
  val orderHandler =
    reactive.system.actorOf(Props[OrderProcessor](), "orderHandler")
  val dispatcher =
    reactive.system.actorOf(Props[CourierDispatcher](), "dispatcher")
  val kitchen = system.actorOf(
    Kitchen.props(Kitchen.TurkishCousine, 2),
    s"${KitchenActorName}_${Kitchen.TurkishCousine}"
  )
