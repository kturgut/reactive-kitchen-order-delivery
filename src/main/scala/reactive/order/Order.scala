...
case class OrderOnFile(
  ...
  override def toString: String =
    s"Order (name:$name, temp:$temp, shelfLife:$shelfLife secs, decayRate:$decayRate, id: $id)"
}

case object OrderOnFile {
  implicit val orderJsonFormat: RootJsonFormat[OrderOnFile] = jsonFormat5(
    OrderOnFile.apply
  )
}
...
sealed trait Temperature extends Product with Serializable {
...
case object Order {
  ...
  def fromOrderOnFile(orderOnFile: OrderOnFile, customer: ActorRef): Order = {
    Order(
      ...
      orderOnFile.name,
      orderOnFile.temp,
      orderOnFile.shelfLife,
      orderOnFile.decayRate,
      customer
    )
  }
}
...
case class OrderLifeCycle(
  ...
  def update(
    deliveryUpdate: DeliveryComplete,
    log: LoggingAdapter
  ): OrderLifeCycle = {
    ...
    if (discard.isDefined) {
      log.error(
        s"This order ${order.id} is already delivered on ${delivery.get.time}, Ignoring discard notice"
      );
      this
  ...
  def update(
    ...
    if (delivery.isDefined) {
      log.error(
        s"This order ${order.id} has been delivered on ${delivery.get.time}, Ignoring discard notice"
      );
      this
