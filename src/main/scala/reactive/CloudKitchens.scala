...
object CloudKitchens {
  ...
  val componentNames = Seq(
    OrderProcessorActorName,
    KitchenActorName,
    CourierDispatcherActorName,
    CustomerSimulatorActorName
  )

  val MaxNumberOfOrdersPerSecond = 2 // TODO read this from config
  def numberOfCouriersNeeded =
    MaxNumberOfOrdersPerSecond * 10 // TODO read from config
...
class CloudKitchens extends Actor with ActorLogging with Stash {
  ...
  def openForService(components: Map[String, ActorRef]): Receive = {

    case StartComponent(name) => startComponent(name, components)

    case Shutdown =>
      log.info("Gracefully Shutting down CloudKitchens!")
      componentNames.filter(components.contains).foreach {
    ...
    case RunSimulation(ordersPerSecond, shelfLifeMultiplier) =>
      runSimulation(components, shelfLifeMultiplier, ordersPerSecond)
  ...
  def runSimulation(
    ...
    numberOfOrdersPerSecond: Int
  ) = {
    system.scheduler.scheduleOnce(100.milliseconds) {
      log.info(
        s"Starting Order Simulation with components: [${components.keys.mkString(",")}]"
      )
      for {
        orderHandler <- components.get(OrderProcessorActorName)
        simulator <- components.get(CustomerSimulatorActorName)
      } yield {
        simulator ! SimulateOrdersFromFile(
          orderHandler,
          numberOfOrdersPerSecond,
          shelfLifeMultiplier
        )
      }
    }
  ...
  def startComponent(name: String, components: Map[String, ActorRef]): Unit = {
    ...
    val child = name match {
      case CustomerSimulatorActorName =>
        context.actorOf(Props[Customer](), CustomerSimulatorActorName)
      case OrderProcessorActorName =>
        context.actorOf(Props[OrderProcessor](), OrderProcessorActorName)
      ...
      case CourierDispatcherActorName =>
        context.actorOf(Props[CourierDispatcher](), CourierDispatcherActorName)
      case name =>
        throw new IllegalArgumentException(s"Unknown component name:$name")
    }
    log.info(s"Starting component $name at path ${child.path}")
    context.watch(child)
    val withNewComponent = components + (name -> child)
    if (componentNames.forall(withNewComponent.contains)) {
  ...
  def initializeComponents(components: Map[String, ActorRef]): Unit =
    components.keys.foreach(name =>
      (name, components.get(name)) match {
        case (KitchenActorName, Some(kitchen)) =>
          kitchen ! Kitchen.InitializeKitchen(
            components(OrderProcessorActorName),
            components(CourierDispatcherActorName)
          )
        case _ =>
      }
    )
