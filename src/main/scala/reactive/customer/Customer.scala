...
/** Customer is a stateless Actor.
  ...
  *   Couriers which are assigned to delivery the PackagedProduct that is created for the
...
object Customer {
  ...
  case class SimulateOrdersFromFile(
    orderHandler: ActorRef,
...
class Customer extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    case SimulateOrdersFromFile(
          orderHandler,
          maxNumberOfOrdersPerSec,
          shelfLifeMultiplier
        ) =>
      simulateOrdersFromFile(
        orderHandler,
  ...
  def signatureWithTip(order: Order): DeliveryAcceptance = {
    if (
      Duration
        .between(order.createdOn, LocalDateTime.now())
        .toMillis < customerHappinessThresholdInMilliseconds
    )
  ...
  def simulateOrdersFromFile(
    ...
    val ordersFromFileSource = FileIO
      .fromPath(Paths.get("./src/main/resources/orders.json"))
      .via(JsonReader.select("$[*]"))
      .async
      .map(byteString => byteString.utf8String.parseJson.convertTo[OrderOnFile])
      .map(order =>
        order.copy(shelfLife = (order.shelfLife * shelfLifeMultiplier).toInt)
      )
