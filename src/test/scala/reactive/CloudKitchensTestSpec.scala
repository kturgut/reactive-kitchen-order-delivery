...
trait TestSpecHelper {
  ...
  def samplePackagedProduct(
    ...
    PackagedProduct(
      Order(id.toString, "Ayran", temp, shelfLife, decayRate, customer),
      time
    )

  def samplePackagedProductAndAssignment(
    id: Int,
    customer: ActorRef,
    courier: ActorRef
  ): (PackagedProduct, CourierAssignment) = {
  ...
  def assertEquals(
    ...
    a.size == b.size &&
    (a zip b forall { ab =>
      math.abs(ab._1._1 - ab._2._1) < RoundingErrorThreshold && math.abs(
        ab._1._2 - ab._2._2
