...
class StorageSpec extends BaseSpec with StorageHelper {
  ...
  "A Shelve" should {
    ...
    val product1 =
      samplePackagedProduct(1, customer.ref, 1000, 0.5f, Cold, time)
    val product2 =
      samplePackagedProduct(2, customer.ref, 2000, 0.5f, Cold, time)
    val product3 =
      samplePackagedProduct(3, customer.ref, 3000, 0.5f, Cold, time)
    val product4 =
      samplePackagedProduct(4, customer.ref, 3000, 0.5f, Cold, time)
    ...
    "store multiple products and preserve priority order" in {
      ...
      assert(shelf.size == 1)
      assert(
        shelf.highestValueProduct == product2 && shelf.lowestValueProduct == product2
      )

      shelf += product1
      assert(shelf.size == 2)
      assert(
        shelf.highestValueProduct == product2 && shelf.lowestValueProduct == product1
      )
      assert(shelf.hasAvailableSpace && !shelf.overCapacity)
      ...
      assert(shelf.size == 3)
      assert(
        shelf.highestValueProduct == product3 && shelf.lowestValueProduct == product1
      )
      assert(!shelf.hasAvailableSpace && !shelf.overCapacity)
      ...
      assert(shelf.size == 4)
      assert(
        shelf.highestValueProduct == product4 && shelf.lowestValueProduct == product1
      )
      assert(!shelf.hasAvailableSpace && shelf.overCapacity)
  ...
  "A Storage" should {
    ...
    val time = LocalDateTime.now()
    val shortLifeProduct =
      samplePackagedProduct(1, customer.ref, 1, 0.99f, Hot, time)
    val longLifeProduct =
      samplePackagedProduct(2, customer.ref, 1000, 0.5f, Hot, time)
    ...
    "store a single product and get it back before it expires" in {
      ...
      shelves.pickupPackageForOrder(longLifeProduct.order) match {
        case Left(Some(product)) =>
          ...
          assert(
            product.remainingShelfLife < longLifeProduct.remainingShelfLife
          )
        case Left(None) => fail("Should have found the product")
        case Right(discard: DiscardOrder) =>
          fail("Product should not have expired")
      }
    }
    ...
    "should be able to age products on its shelves over time.. without optimization kicking in" in {
      ...
      hots.foreach(product =>
        storage.putPackageOnShelf(product, product.createdOn)
      )
      ...
      val expected = {
        Map(
          0 -> Map(
            Temperature.Hot -> List(
              ("0", 0.0f, 1.0f),
              ("1", 1.0f, 1.0f),
              ("2", 2.0f, 1.0f)
            ),
            Temperature.All -> List(("3", 3.0f, 1.0f), ("4", 4.0f, 1.0f))
          ),
          500 -> Map(
            Temperature.Hot -> List(
              ("0", 0.0f, 0.0f),
            ...
            Temperature.All -> List(("3", 2.0f, 0.6666667f), ("4", 3.0f, 0.75f))
          ),
          1000 -> Map(
            Temperature.Hot -> List(
              ("0", 0.0f, 0.0f),
            ...
            Temperature.All -> List(("3", 1.0f, 0.33333334f), ("4", 2.0f, 0.5f))
          )
        )
      }
    ...
    "should not overflow" in {
      ...
      val hots: IndexedSeq[PackagedProduct] = for (i <- 0 until 10) yield {
        samplePackagedProduct(
          ...
          0.5f,
          Hot,
          time.plus(i * 10, ChronoUnit.MILLIS)
        )
      }
      hots.foreach { product =>
        storage.putPackageOnShelf(product, product.createdOn)
...
trait StorageHelper {
  ...
  def collectStorageData(
    storage: Storage
  ): Map[Temperature, List[(String, Float, Float)]] =
  ...
  def report(storage: Storage, verbose: Boolean = false): Unit = {
    def print(shelf: Shelf): Unit = {
      println(
        s"${shelf.name} shelf capacity utilization:${shelf.products.size} / ${shelf.capacity}, decay rate modifier:${shelf.decayModifier} createdOn: ${storage.createdOn
          .format(dateFormatter)}"
      )
      if (verbose)
        println(s"       contents: ${shelf.products
          .map(product => (product.order.id, product.order.shelfLife, product.order.name, product.value, product.createdOn.format(dateFormatter)))
          .mkString(",")}")
      else
        println(
          s"       contents: ${shelf.products.map(product => (product.order.id, product.remainingShelfLife, product.value)).mkString(",")}"
        )
    }
